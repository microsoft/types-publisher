...
const registryOutputPath = util_1.joinPaths(settings_1.outputDirPath, typesRegistry);
...
async function publishRegistry(dt, allPackages, dry, client) {
    ...
    const { npmVersion, highestSemverVersion, npmContentHash, lastModified } = await fetchAndProcessNpmInfo(typesRegistry, client);
...
async function publish(client, packageName, packageJson, version, dry, log) {
...
async function installForValidate(log) {
    ...
    const cmd = `node ${npmPath} install types-registry@next ${io_1.npmInstallFlags}`;
    log(cmd);
    const err = (await util_1.execAndThrowErrors(cmd, settings_1.validateOutputPath)).trim();
    if (err) {
        console.error(err);
    }
}
...
async function validate(log) {
    ...
    const output = util_1.joinPaths(registryOutputPath, "index.json");
    const nodeModules = util_1.joinPaths(validateTypesRegistryPath, "index.json");
    log(`Checking that ${output} is newer than ${nodeModules}`);
    assertJsonNewer(await io_1.readJson(output), await io_1.readJson(nodeModules), log);
}
async function validateIsSubset(notNeeded, log) {
    await installForValidate(log);
...
function assertJsonNewer(newer, older, log, parent = "") {
...
function generatePackageJson(name, registryName, version, typesPublisherContentHash) {
    const json = {
        name,
        version,
...
async function fetchAndProcessNpmInfo(escapedPackageName, client) {
    ...
    const npmVersion = versions_1.Semver.parse(util_1.assertDefined(info.distTags.get("latest")));
