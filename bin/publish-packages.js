    ...
    for (const cp of changedPackages.changedTypings) {
        ...
        if (commits.length > 0) {
            ...
            if (dry) {
                ...
                const commented = await postGithub(`repos/DefinitelyTyped/DefinitelyTyped/issues/${latestPr}/comments`, { body: "`" + cp.pkg.fullNpmName + "@" + cp.pkg.major + "." + cp.pkg.minor + "` is now published." }, githubAccessToken, fetcher);
