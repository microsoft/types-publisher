...
async function publishTypingsPackage(client, changedTyping, dry, log, registry) {
    ...
    if (latestVersion !== undefined) {
        ...
        const name = registry === common_1.Registry.Github ? pkg.fullGithubName : pkg.fullNpmName;
...
async function deprecateNotNeededPackage(client, pkg, dry = false, log, registry) {
    const name = registry === common_1.Registry.Github ? pkg.fullGithubName : pkg.fullNpmName;
    if (dry) {
...
async function updateTypeScriptVersionTags(pkg, version, client, log, dry, registry) {
