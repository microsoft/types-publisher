// Modified from https://raw.githubusercontent.com/palantir/tslint/master/src/rules/onlyArrowFunctionsRule.ts
// TODO: pull request
...
function usesThisInBody(node: ts.Node): boolean {
	if (node.kind === ts.SyntaxKind.ThisKeyword ||
		node.kind === ts.SyntaxKind.Identifier && (node as ts.Identifier).text === "arguments") {
		return true;
	}
	if (hasNewThis(node)) {
		return false;
