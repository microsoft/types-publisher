import { AnyPackage, TypingsData, DefinitionFileKind, Log, fullPackageName, isNotNeededPackage, mkdir, getOutputPath, notNeededReadme, settings } from './common';
		...
		if (shouldUpdate) {
			...
			if (runCommand('Publish', log, dry, args)) {
				if (isNotNeededPackage(pkg)) {
					...
					const args = ['npm', 'deprecate', fullPackageName(pkg.typingsPackageName), JSON.stringify(message)];
